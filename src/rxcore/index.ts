...
export class RXCore {
    ...
    public static importPages(file: File, pageRange: number[][], pageArray: number[][], isReplace: boolean, count: number, width: number, height: number): Promise<any> {
        return RxCore.importPages(file, pageRange, pageArray, isReplace, count, width, height)
    }

    public static zoomMarkup(markup: IMarkup): Promise<void> {
        return RxCore.zoomWithMarkup(markup);
    }
